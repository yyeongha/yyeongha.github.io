...
# 1. Introduction
2010년대 컴퓨터 비전 분야는 이미지 인식에 탁월한 합성곱 신경망(ConvNets)의 발전으로 급성장했습니다. 

2020년, 자연어 처리 분야의 트랜스포머 모델이 ViT(Vision Transformer)라는 이름으로 컴퓨터 비전에 도입되면서 새로운 변화를 맞이했습니다. ViT는 이미지 분류에는 뛰어났지만 객체 감지나 이미지 분할과 같은 작업에는 한계를 보였습니다. 

이후 Swin Transformer와 같은 계층적 트랜스포머 모델이 등장하여 ConvNets의 장점을 일부 차용하며 다양한 컴퓨터 비전 작업에서 좋은 성능을 보였습니다. 

본 논문에서는 ConvNets를 개선하여 트랜스포머 모델만큼의 성능을 내는 ConvNeXt라는 새로운 모델을 제안합니다. ConvNeXt는 다양한 벤치마크에서 Swin Transformer와 비슷하거나 더 나은 성능을 보였고, 기존 ConvNets의 단순함과 효율성도 유지했습니다. 본 연구는 ConvNets가 여전히 컴퓨터 비전 분야에서 중요한 역할을 할 수 있음을 보여주며, 컴퓨터 비전 모델 설계에 새로운 시각을 제시합니다. 
...
ImageNet-1K, ImageNet-22K 모두 convnext가 더 좋은 성능을 나타내고 두개 모두 좋은 성능을 나타냈다는 점은 기존의 ViT가 CNN과의 차별점으로 하는 것은 사전학습 데이터의 양이 많을수록 그만큼 성능향상의 폭이 CNN보다 크다라는 것을 언급했는데, 본 논문에서는 convnext가 그러한 점을 ViT보다 더 좋게 나타내고 있다고 볼 수 있다.
각 버블의 면적은 모델 제품군의 변형 FLOP에 비례합니다. 

* 최근 ViT와 비교하는 CNN은 너무 올드하다
* CNN을 최신화(modernizing)하면 얼마나 성능이 향상될까? (ResNet을 Swin transform에 맞게 변형)
* ViT와 유사한 조건으로 CNN을 학습 및 구성하면 성능이 더 향상하는 것을 보임
* 최근 학습 및 모델 구조 변화에 대한 흐름을 파악할 수 있다.
...
본 논문에서는 ResNet-50/200 모델을 시작으로 단계별로 모델 구조를 변경하며 성능을 개선하는 과정을 거쳐, 최종적으로 ConvNeXt라는 새로운 ConvNet 모델을 제안합니다. 각 단계는 다음과 같습니다.
...
2) macro design
* stem cell을 ViT 스타일의 "patchify" layer로 변경합니다. 
ResNet-50의 경우 stage 마다 블록의 갯수가 3,4,6,3개 이렇게 들어있는데, swin-T의 경우 1:1:3:1로 들어있다. 그래서 ResNet에도 이 비율을 맞춰 3,3,9,3개로 바꿈.  
-> accuracy : 78.8% -> 79.4% (+0.6%)

![macro]()

* 기존 ResNet은 stride2 의 7x7 convolution에 max pool layer을 통해 이미지를 4x4로 downsampling하는 것에 반해, 이를 swin이 가지고 있는 형태의 4x4형태의 non-overlapping convolution의 patchify stem으로 대체
...
3) ResNeXt
* ResNeXt의 아이디어를 적용하여 depthwise convolution을 사용 
  * 사용한 이유는 Transformer의 self-attention에서의 weighted sum operation과 유사해서
* 네트워크의 너비를 증가시킵니다. 
-> accuracy : 79.4% -> 79.5% (+0.1%)

4) Inverted Bottleneck
* Inverted Bottleneck 디자인을 적용하여 hidden dimension을 4배 늘립니다. 
